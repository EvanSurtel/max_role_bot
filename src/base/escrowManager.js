...
/**
 * Compute per-winner USDC shares with remainder going to first winner.
 * BigInt division truncates; giving the remainder to winner[0] keeps
 * totalPayout === matchPrize exactly (no dust stranded in escrow).
 * Returns an array aligned with winningPlayerIds: `[{userId, address,
 * amount}...]` plus error rows for winners with no wallet row.
 */
function _buildDisbursements(winningPlayerIds, totalPotUsdc) {
...
async function disburseWinnings(matchId, challengeId, winningPlayerIds, totalPotUsdc, { fromDispute = false } = {}) {
  ...
  for (const d of disbursements) {
    if (d.error) continue;
    winnerAddresses.push(d.address);
    winnerAmounts.push(d.amount);
  }
  ...
  // completed. If the DB credit throws for some reason we leave the
  // row as 'pending_onchain' so the poller can fix it up when the
  // delta is observed on-chain.
  for (const d of disbursements) {
