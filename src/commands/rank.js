...
async function buildRankCard(targetUser, lang = 'en') {
  ...
  // Append sub-tier (I / II / III) only for tiers that have sub-tiers
  // (Bronze through Sentinel). Obsidian and Top 10 are flat — no
  // roman numeral. computeSubTier returns subTier=null for flat tiers
  ...
  if (!tier.topN) {
    ...
    if (sub.subTier != null) rankName = `${baseTierName} ${['I', 'II', 'III'][sub.subTier - 1]}`;
  }
